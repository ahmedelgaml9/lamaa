...
After installing the `Laravel Payfort` library, register the `BadeePayfort\Providers\PayfortServiceProvider` 
...
'providers' => [
    ...
    BadeePayfort\Providers\PayfortServiceProvider::class,
],
```

Also, add the `Payfort` facade to the `aliases` array in your `app` configuration file:
```php
'Payfort' => BadeePayfort\Facades\Payfort::class
```

After that, run the following command to publish the configurations file:
```
$ php artisan vendor:publish --provider "BadeePayfort\Providers\PayfortServiceProvider"
```
...
```
use BadeePayfort\Traits\PayfortResponse as PayfortResponse;
