               ...
               <table class="table align-middle table-row-dashed fs-6 gy-5" id="db_table">
                  <thead>
                       <tr class="text-start text-muted fw-bolder fs-7 text-uppercase gs-0">
                            ...
                            <th class="min-w-125px">{{__('admin.region')}}</th>
                   ...
                   <tbody class="text-gray-600 fw-bold">
                    ...
                    <tr>
                       <td>{{ ++$key}}</td>
                       <td>{{$city->name}}</td>
                          @if(isset($city->region))<td>{{$city->region->name}}</td>@else<td>المنطقة محذوفة</td>@endif
                        <td class="text-end">
                            <a href="#" class="btn btn-light btn-active-light-primary btn-sm" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end" data-kt-menu-flip="top-end">{{__('admin.actions')}}
                              ...
                              <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                                <div class="menu-item px-3">
                                   <a href="{{route('cities.edit', $city->id)}}" class="menu-link px-3">{{__('admin.edit')}}</a>
                                </div>
                                  <div class="menu-item px-3">
                                       <a href="#" class="menu-link px-3" data-bs-toggle="modal" data-bs-target="#kt_modal_1{{$city->id}}">{{__('admin.delete')}}</a>               
                                  </div>
                                </div>
                           </td>
                       </tr>
 
                            <div id="kt_modal_1{{$city->id}}"  tabindex="-1" class="modal fade">
                                              ...
                                              <div class="modal-body text-center">
                                               ...
                                               <div class="text-center pt-15">
                                                <button type="button" class="btn btn-light" data-bs-dismiss="modal" style="position:relative;float:right;">{{__('admin.Close')}}</button>
                                                 {!! Form::open(['url' => route('cities.destroy', $city->id), 'method' => 'delete']) !!}
