     ...
     <div class="d-flex align-items-center py-1">
         <button type="button" class="btn btn-light-primary me-3" data-bs-toggle="modal" data-bs-target="#subscriptions">
            ...
            {{__('admin.filter')}}</button>
              ...
              <a href="{{route('export_newcustomers', request()->query())}}" class="btn btn-sm btn-flex btn-dark btn-active-primary fw-bolder"><i class="fa fa-file-download"></i>تصدير</a>

              <label  class="col-form-label  fw-bold fs-6"></label>
              @if(request()->query() == null)<a href="{{url('admin/newcustomers/pdf')}}"  target="_blank" class="btn btn-sm btn-danger"><i class="fa fa-print"></i></a>@endif
             </div>
       @endsection
       @section('content')
  <div class="d-flex flex-wrap flex-stack pb-7">
     <div class="d-flex flex-wrap align-items-center my-1">
            <div class="d-flex align-items-center position-relative my-1">
                <form action="{{route('newcustomers_reports')}}" id="search_with_word_form" style="display: inline-flex">
										...
										<div class="modal-content">
											<div class="modal-header">
									         ...
									         <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
                                                    <form action="{{route('newcustomers_reports')}}" class="px-7 py-5" method="get">
                                                    @csrf
                                                    ...
                                                    <div class="fv-row mb-10">         
                                                     ...
                                                     <div class="fv-row mb-10">         
                                                         <label  class=" col-form-label fw-bold fs-6">{{__('admin.email')}}</label>
                                                         <input type="text" class="form-control form-control-solid" name="email" placeholder="{{__('admin.email')}}" />
											     	</div>

                                                        <div class="fv-row mb-10">
                                                            <label class="form-label fw-bold">{{__('admin.cities')}}</label>
                                                                <select class="form-select form-select-solid" name="city" >
                                                              ...
                                                              <div class="fv-row mb-10">
                                                                  ...
                                                                  <select class="form-select form-select-solid" name="status" >
                                                                        <option></option>
                                                                        @foreach(customerStatusesList() as $key => $value)
                                                               ...
                                                               <div class="fv-row mb-10">
                                                                     <label class="form-label fw-bold">{{__('trans_admin.cart_rate')}}:</label>
                                                                      <div>
                                                                         <select class="form-select form-select-solid"  name="cart_rate">
                                                                            <option></option>
                                                                            @foreach(customerCartRates() as $key => $value)
                                                                                <option value="{{$key}}" @if($key == request()->get('cart_rate')) selected @endif>{{$value}}</option>
                                                                            @endforeach
                                                                        </select>
                                                                    </div>
                                                                </div>
