...
@section('content')
             <div class="card card-xl-stretch mb-5 mb-xl-10">
                   <div class="card-header border-0 pt-5">
                     ...
                     @if($data->id)
                         {!! Form::open(['url' => route('delegates.update', $data->id), 'method' => 'put', 'files'=>true]) !!}
                     @else
                          {!! Form::open(['url' => route('delegates.store'), 'method' => 'post', 'files'=>true]) !!}
                     @endif
                                    ...
                                    <div class="col-md-6">
                                         <label class="form-label fs-6 fw-bolder text-gray-700 mb-3">{{__('admin.phone')}}</label>
                                            <div class="mb-5 {{ $errors->has('mobile') ? 'has-error' : '' }}">
                                                  <input type="number" name="mobile" class="form-control form-control-solid" value="{{old('mobile',$data->mobile)}}" placeholder="966xxxxx"/>
